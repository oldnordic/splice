...
fn test_magellan_integration_open() {
    ...
    assert!(integration.is_ok(), "MagellanIntegration::open should succeed");
    ...
    println!("MagellanIntegration opened successfully at: {}", db_path.display());
}
...
"#;
    ...
    assert!(!rust_symbols.unwrap().is_empty(), "Should find indexed Rust symbols");
}
...
"#;
    ...
    assert!(chunk_content.is_some(), "Should have code chunk at the symbol's span");
...
"#;
    ...
    assert!(result.is_ok(), "get_code_chunks_for_symbol should succeed: {:?}", result.err());
...
fn test_magellan_symbol_info_format_compatibility() {
    ...
    assert!(!symbol.file_path.is_empty(), "Symbol must have 'file_path' field");
...
fn test_magellan_code_chunk_format_compatibility() {
    ...
    if !chunks.is_empty() {
        ...
        assert!(!chunk.content.is_empty(), "CodeChunk must have 'content' field");
        assert!(
            chunk.byte_start < chunk.byte_end,
...
fn test_magellan_get_code_chunk_nonexistent_file() {
    ...
    assert!(result.is_ok(), "get_code_chunk should succeed for non-existent file");
...
fn test_magellan_get_code_chunk_invalid_range() {
    ...
    assert!(result.is_ok(), "get_code_chunk should succeed for invalid range");
    let chunk_opt = result.unwrap();

    // Either None (no chunk) or Some with content (Magellan may handle this)
    let _ = chunk_opt;

    println!("get_code_chunk handled invalid byte range: {:?}", chunk_opt.is_some());
}
...
fn test_magellan_get_code_chunk_inverted_range() {
    ...
    assert!(result.is_ok(), "get_code_chunk should handle inverted range");
...
fn test_magellan_get_code_chunks_nonexistent_symbol() {
    ...
    assert_eq!(chunks.len(), 0, "Non-existent symbol should return empty chunks");
...
"#;
    ...
    assert!(!python_symbols.unwrap().is_empty(), "Should find Python symbols");
...
"#;
    ...
    assert!(!ts_symbols.unwrap().is_empty(), "Should find TypeScript symbols");
