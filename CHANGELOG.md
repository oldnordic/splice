...
- **Semantic search** (`--semantic-query`) for `find`, `reachable`, `dead-code`, and `slice` commands:
  - Resolves natural-language queries to symbol names via HNSW embeddings (generated by Magellan, searched by Ollama).
  - `splice find --semantic-query "function that handles auth" --db .magellan/splice.db`
  - `splice reachable --semantic-query "error handling logic" --direction forward`
  - `splice dead-code --semantic-query "main entry point"`
  - `splice slice --semantic-query "database connection pool" --direction backward`
  - Requires Magellan-generated HNSW index (`magellan embed --db <db>`) and Ollama config in `~/.config/magellan/config.toml`.
  - Graceful fallback: if no HNSW index exists, commands report a clear error without breaking normal functionality.
- New `splice::query::semantic` module with helpers:
  - `embed_query()` — embeds text via Ollama/OpenAI-compatible API.
  - `read_magellan_config()` — reads embedding settings from Magellan config.
  - `resolve_semantic_query()` — returns top-k `(entity_id, distance)` matches from HNSW index.
  - `resolve_symbol_by_entity_id()` — looks up `SymbolInfo` from graph database by entity ID.
  - `resolve_semantic_to_symbol()` — resolves a query to a single best-matching symbol.
  - `resolve_semantic_to_symbols()` — resolves a query to multiple matching symbols.

### Changed

- CLI argument structs updated with `semantic_query: Option<String>` on `Find`, `Reachable`, `DeadCode`, and `Slice`.
- Command handlers (`execute_find`, `execute_reachable`, `execute_dead_code`, `execute_slice`) accept an optional `semantic_query` parameter and resolve it before falling back to explicit symbol names.
