...
@src/output.rs (SpanContext type from 11-01)
@src/error.rs
</context>
...
Note: Move the SpanContext struct definition from output.rs to context.rs, and update output.rs to import it.
  </action>
...
<task type="auto">
  <name>Task 2: Move SpanContext to context.rs and update imports</name>
  <files>src/output.rs, src/context.rs, src/lib.rs</files>
  <action>
1. In src/context.rs, keep the SpanContext struct (defined in Task 1).

2. In src/output.rs:
   - Remove the SpanContext struct definition (it's now in context.rs)
   - Add: `use crate::context::SpanContext;` at the top

3. In src/lib.rs:
   - Add: `pub mod context;` after pub mod checksum
   - Add: `pub use context::{extract_context, SpanContext};` after the checksum re-exports

This way SpanContext is properly exported from the library root.
  </action>
  <verify>cargo check --lib succeeds</verify>
  <done>SpanContext is exported from lib.rs and imported in output.rs</done>
</task>
...
2. SpanContext is properly exported from lib.rs
